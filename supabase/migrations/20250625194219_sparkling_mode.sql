/*
  # Fix extension_in_public Warning for Vector Extension

  1. Purpose:
    - Move the 'vector' extension from the 'public' schema to a dedicated 'extensions' schema
    - Address the "Extension in Public" security warning

  2. Steps:
    - Create dedicated extensions schema
    - Check if vector extension exists in public schema
    - Move vector extension to extensions schema safely
    - Update search_path to include extensions schema
    - Handle column references to vector type

  3. Note:
    - This migration takes a conservative approach to avoid data loss
    - May require table recreation with ALTER TABLE statements for existing vector columns
*/
...
BEGIN
  ...
  ALTER DATABASE CURRENT_SET search_path TO "$user", public, extensions;
END $$;

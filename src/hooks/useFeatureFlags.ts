...
export function useFeatureFlags() {
  ...
  const useAllFeatureFlags = () => {
    return useQuery({
      ...
      queryFn: async (): Promise<FeatureFlag[]> => {
        try {
          console.log('Loading feature flags from Supabase...');
          ...
          // Type assertion since our mock client returns the correct structure
          return (data as FeatureFlag[]) || getInitialMockFlags();
        } catch (err) {
          console.warn('Error loading feature flags, using mock data:', err);
          return getInitialMockFlags();
        }
      },
      enabled: !!user,
  ...
  const createFeatureFlag = useMutation({
    mutationFn: async (flag: Omit<FeatureFlag, 'id' | 'created_at' | 'updated_at'>) => {
      // For now, just add to cache since we're using mock data
      ...
      console.log('Created feature flag:', newMockFlag);
      return newMockFlag;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['feature-flags-admin'] });
  ...
  const updateFeatureFlag = useMutation({
    mutationFn: async (flag: Partial<FeatureFlag> & { id: string }) => {
      // For now, just update in cache since we're using mock data
      const currentFlags = queryClient.getQueryData<FeatureFlag[]>(['feature-flags-admin']) || getInitialMockFlags();
      ...
      console.log('Updated feature flag:', flag);
      return updatedFlags.find(f => f.id === flag.id);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['feature-flags-admin'] });
      queryClient.invalidateQueries({ queryKey: ['feature-flags-available'] });
  ...
  const deleteFeatureFlag = useMutation({
    mutationFn: async (id: string) => {
      ...
      queryClient.setQueryData(['feature-flags-admin'], updatedFlags);
      console.log('Deleted feature flag with id:', id);
      return { id };
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['feature-flags-admin'] });

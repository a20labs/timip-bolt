...
import { shouldUseMockAuth, getOAuthErrorMessage } from '../../config/socialAuth';
...
export function AuthPage() {
  ...
  // Handle error messages from URL parameters (e.g., from OAuth callback)
  useEffect(() => {
  ...
  const handleSocialAuth = async (provider: 'google' | 'facebook' | 'linkedin_oidc' | 'spotify') => {
    setLoading(true);
    setMessage('');

    try {
      // Check if we should use mock for this provider
      const useMock = shouldUseMockAuth(provider);

      if (useMock) {
        // Mock social authentication for demo or unconfigured providers
        console.log(`🔐 Starting mock social auth for ${provider}`);
        setMessage(`Redirecting to ${provider}...`);
        
        // Simulate OAuth redirect by navigating to callback with mock data
        setTimeout(() => {
          // Mock the OAuth callback with success parameters
          window.location.href = `/auth/callback?provider=${provider}&access_token=mock_token_${Date.now()}&mock_auth=true`;
        }, 1500);
        return;
      }

      // Real Supabase OAuth for configured providers
      console.log(`🔐 Starting real OAuth for ${provider}`);
      ...
      if (error) {
        // If the provider is not enabled, fall back to mock authentication
        if (error.message?.includes('not enabled') || error.message?.includes('Unsupported provider')) {
          console.log(`🔐 Provider ${provider} not enabled, falling back to mock auth`);
          const friendlyMessage = getOAuthErrorMessage(provider, error.message);
      ...
      if (data?.url) {
        // For real OAuth, redirect to the provider's authorization URL
        setMessage(`Redirecting to ${provider}...`);
      ...
      // If it's a provider configuration error, fall back to mock
      if (errorMessage.includes('not enabled') || errorMessage.includes('Unsupported provider')) {
        console.log(`🔐 Falling back to mock auth for ${provider} due to configuration error`);
        const friendlyMessage = getOAuthErrorMessage(provider, errorMessage);
        setMessage(friendlyMessage);
      ...
      setMessage(getOAuthErrorMessage(provider, errorMessage));
      setLoading(false);
    }
  };

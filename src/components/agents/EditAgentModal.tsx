...
import { X, Save, Bot, Upload, Trash2, Image } from 'lucide-react';
...
export function EditAgentModal({ isOpen, onClose, agent, onSave, onDelete }: EditAgentModalProps) {
  ...
  useEffect(() => {
    if (agent) {
      setFormData({
        ...
        voice_id: agent.voice_id || 'eleven-labs-rachel',
        logic_level: agent.logic_level || 'MEDIUM',
      });
      setAvatarPreview(agent.avatarUrl || null);
      setHeaderPreview(agent.headerUrl || null);
      setSkills(agent.skills || [{ name: '', value: 90 }]);
    }
  }, [agent]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const updatedAgent = {
      ...agent,
      ...formData,
      avatarUrl: avatarPreview || agent.avatarUrl,
      headerUrl: headerPreview || agent.headerUrl,
